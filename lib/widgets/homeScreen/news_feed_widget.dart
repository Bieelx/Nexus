...
class _NewsCacheItem {
  ...
  NewsItem toNewsItem() => NewsItem(
      ...
      createdAt: createdAt,
      points: points);
  static _NewsCacheItem fromNewsItem(NewsItem n) => _NewsCacheItem(
      title: n.title,
      url: n.url,
      source: n.source,
...
class _NewsFirestoreCache {
  ...
  static Future<void> write(NewsFilter f, List<NewsItem> items) async {
    try {
      final maps =
          items.map(_NewsCacheItem.fromNewsItem).map((e) => e.toMap()).toList();
...
class _NewsFeedWidgetState extends State<NewsFeedWidget> {
  ...
  Future<List<NewsItem>> _getWithCache(NewsFilter f) async {
    ...
    if (cached != null && cached.isNotEmpty) {
      Future(() async {
        try {
          final fresh = (f == NewsFilter.todos)
              ? await _fetchAll()
              : await _fetchNews(f.query);
    ...
    final fresh =
        (f == NewsFilter.todos) ? await _fetchAll() : await _fetchNews(f.query);
    await _NewsFirestoreCache.write(f, fresh);
    return fresh;
  }
  ...
  Future<List<NewsItem>> _fetchAll() async {
    ...
    if (merged.isEmpty) {
      final uriTop = Uri.parse('https://newsapi.org/v2/top-headlines').replace(
          queryParameters: {
            'country': 'br',
            'pageSize': '30',
            'apiKey': _NEWSAPI_KEY
  ...
  Future<List<NewsItem>> _fetchNews(String q) async {
    final f = _selected;
    final qTrim = q.trim();
    final uriEverything = Uri.parse('https://newsapi.org/v2/everything')
    ...
    if (result.length < 6) {
      ...
      final uriTop = Uri.parse('https://newsapi.org/v2/top-headlines')
          .replace(queryParameters: paramsTop);
      final merged = [
        ...result,
        ..._filterLocal(await _loadArticles(uriTop), f)
      ];
      final seen = <String>{};
      result = merged
          .where((n) => seen.add((n.url ?? n.title).toLowerCase()))
          .toList();
    }
    result.sort((a, b) =>
  ...
  Future<List<NewsItem>> _loadArticles(Uri uri) async {
    ...
    return articles.map<NewsItem>((a) {
      return NewsItem(
        ...
        createdAt: a['publishedAt'] is String
  ...
  Future<void> _onTapSummarizeAI() async {
    ...
    String summary;
    try {
      final itemsForAI = _lastItems
          .map((n) => newsai.NewsItem(
              title: n.title,
              source: n.source,
      ...
      summary = await newsai.NewsAIService.summarize(
          items: itemsForAI, filterLabel: _currentFilterLabel);
    } catch (e) {
      summary =
  ...
  Widget build(BuildContext context) {
    const box = Color(0xB2515767);
    const border = Color(0xFF7884C4);
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: const BoxDecoration(
                  color: Color(0xFF6C7691), shape: BoxShape.circle),
              padding: const EdgeInsets.all(4),
              child: SvgPicture.asset('assets/icons/newsAI.svg',
                  fit: BoxFit.contain,
                  colorFilter: const ColorFilter.mode(
                      Color(0xFFA259FF), BlendMode.srcIn)),
            ),
          ),
        ]),
        ...
        Expanded(
          child: FutureBuilder<List<NewsItem>>(
            ...
            builder: (context, snap) {
              ...
              if (snap.hasError) {
                return Center(
                    child: Padding(
                        ...
                        child: Text(
                            'Não foi possível carregar as notícias agora. ${snap.error}'
              ...
              if (items.isEmpty) {
                return const Center(
                    child: Text('Sem resultados agora.',
                ...
                itemBuilder: (context, i) {
                  ...
                  return Container(
                    decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color: const Color(0xFF6C7691), width: 1)),
                    child: ListTile(
                      dense: true,
            ...
            ...NewsFilter.values.map((f) => Padding(
                  ...
                  child: ChoiceChip(
                    ...
                    labelStyle: TextStyle(
                        color: selected == f
                            ? const Color(0xFFC1A1EC)
                            : Colors.white,
                        fontWeight: FontWeight.w600),
                    side: BorderSide(
                        color: selected == f
                            ? const Color(0xFF6C52BB)
                            : const Color(0xFF6C7691)),
