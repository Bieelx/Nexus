...
class _LoginPageState extends State<LoginPage> with TickerProviderStateMixin {
  ...
  Future<void> _doGoogleLogin() async {
    ...
    try {
      ...
      // Se for bem-sucedido, o AuthCheck fará a navegação
    } on AuthException catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.message)));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Erro ao entrar com Google: $e')));
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }
            ...
            child: Column(
              ...
              children: [
                SizedBox(
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      _floatingIcon(const Icon(Icons.lock_outline, color: Colors.white, size: 18), dx: -28, dy: -40, delay: 0.0),
                      _floatingIcon(const Icon(Icons.bolt, color: Colors.white, size: 18), dx: 44, dy: -22, delay: 0.4),
                      _floatingIcon(const Icon(Icons.security, color: Colors.white, size: 18), dx: -34, dy: 38, delay: 0.8),
                      AnimatedBuilder(
                        animation: _pulseCtrl,
                        builder: (_, __) {
                ...
                FadeTransition(
                  ...
                  child: Column(
                    children: const [
                      Text('Nexus', style: TextStyle(fontSize: 36, fontWeight: FontWeight.w800, color: Colors.white)),
                      SizedBox(height: 6),
                      Text('Sua plataforma de segurança digital', textAlign: TextAlign.center, style: TextStyle(color: Color(0xFFD5C4F3), fontSize: 14)),
                      SizedBox(height: 2),
                      Text('Proteja-se contra ameaças cibernéticas', style: TextStyle(color: Color(0xFFAE85E5), fontSize: 12)),
                    ],
                  ),
                ),
                const SizedBox(height: 24),
                _PrimaryButton(label: 'Fazer login', onTap: () => setState(() => _screen = _Screen.login)),
                const SizedBox(height: 12),
                _OutlineButton(label: 'Criar conta', onTap: () => setState(() => _screen = _Screen.signup)),
                const SizedBox(height: 16),
                const Text('Ao continuar, você concorda com nossos Termos de Uso e Política de Privacidade', textAlign: TextAlign.center, style: TextStyle(color: Color(0xFF717182), fontSize: 11)),
              ],
            ),
          ),
        ),
  ...
  Widget _buildLogin(BuildContext context) {
    return _AuthScaffold(
      ...
      onGoogleTap: _loading ? null : _doGoogleLogin,
      child: Form(
        key: _loginFormKey,
        child: Column(
  ...
  InputDecoration _dec(String label, {IconData? icon, Widget? suffix}) {
    return InputDecoration(
      hintText: label,
      hintStyle: const TextStyle(color: Colors.white54, fontWeight: FontWeight.normal, fontSize: 16),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          gradient: const LinearGradient(colors: [Color(0xFF9240FE), Color(0xFF8523F7)]),
          borderRadius: BorderRadius.circular(16),
          boxShadow: const [BoxShadow(color: Color(0x55000000), blurRadius: 10, offset: Offset(0, 6))],
        ),
        child: Center(
          child: loading
              ? const SizedBox(
