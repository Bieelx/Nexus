...
class EventLogService {
  ...
  static Future<List<SecurityEvent>> _generateLoginEvents(
    ...
    for (var user in users.take(5)) {
      ...
      if (random.nextInt(10) < 2) {
        ...
        events.add(SecurityEvent(
          ...
          description:
              'Múltiplas tentativas de login falharam para usuário ${user['email'] ?? 'unknown'}',
          timestamp:
  ...
  static Future<List<SecurityEvent>> _generateChatSecurityEvents(
    ...
    for (var chat in chats.take(10)) {
      ...
      bool hasSuspiciousContent = suspiciousWords
          .any((word) => message.toLowerCase().contains(word.toLowerCase()));
  ...
  static Future<Map<String, int>> getThreatStatistics() async {
    try {
      ...
      return {
        'malware_attempts':
            todayEvents.where((e) => e.eventType.contains('Malware')).length,
        'failed_logins': todayEvents
            .where((e) => e.eventType.contains('Failed Login'))
            .length,
        'suspicious_activity':
            todayEvents.where((e) => e.eventType.contains('Suspicious')).length,
        'blocked_threats': todayEvents
            .where((e) => e.severity == 'Low' || e.severity == 'Medium')
            .length,
      };
    } catch (e) {
      return {
