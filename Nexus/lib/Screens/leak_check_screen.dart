...
class _LeakCheckerScreenState extends State<LeakCheckerScreen> {
  ...
  Future<void> _checkPasswordLeak(String password) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (found) {
        final count = hashes.firstWhere((line) => line.split(':')[0] == suffix).split(':')[1];
        _resultMessage = '⚠️ Sua senha foi encontrada em vazamentos ($count vezes).';
        _showHelpToast();
      } else {
  ...
  void _showHelpToast() {
    CustomToast.show(
      ...
      onConfirm: () {
        ...
        if (_selectedType == 'Email') autoMsg = "Meu email vazou, o que posso fazer?";
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                '<Vazamentos./>',
                style: TextStyle(color: AppColors.primaryPurple, fontSize: 22, fontFamily: 'JetBrains Mono', fontWeight: FontWeight.w400),
              ),
              ...
              if (_resultMessage.isNotEmpty)
                _StatusBanner(resultMessage: _resultMessage, selectedType: _selectedType),
            ],
          ),
        ),
      ),
...
class _VerificationCard extends StatelessWidget {
  ...
  const _VerificationCard({
    required this.controller, required this.selectedType,
    required this.onDropdownChanged, required this.isLoading,
...
class _StatusBanner extends StatelessWidget {
  ...
  final String selectedType;
  const _StatusBanner({required this.resultMessage, required this.selectedType});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: isLeaked ? const Color(0xFFD07274) : const Color(0xFF678EE6)),
      ),
      child: Row(
        children: [
          Icon(isLeaked ? Icons.warning_amber_rounded : Icons.check_circle_outline, color: isLeaked ? const Color(0xFFD64344) : const Color(0xFFB8CBF4)),
          const SizedBox(width: 12),
          Expanded(child: Text(resultMessage, style: TextStyle(color: Colors.white.withOpacity(0.9), fontWeight: FontWeight.w600))),
        ],
      ),
    );
  }
      ...
      content: Row(
        children: [
          ...
          TextButton(
            ...
            child: const Text('Sim', style: TextStyle(color: AppColors.primaryPurple, fontWeight: FontWeight.bold)),
          ),
          TextButton(
