...
class _NewsFeedWidgetState extends State<NewsFeedWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: box, borderRadius: BorderRadius.circular(16), border: Border.all(color: border, width: 1)),
        ...
        Expanded(
          child: FutureBuilder<List<NewsItem>>(
            ...
            builder: (context, snap) {
              ...
              if (snap.hasError) {
                return Center(child: Padding(padding: const EdgeInsets.all(8.0), child: Text('Não foi possível carregar as notícias agora. ${snap.error}'.replaceAll('Exception: ', ''), style: const TextStyle(color: text), textAlign: TextAlign.center)));
              }
              final items = snap.data ?? const [];
              _lastItems = items;
              if (items.isEmpty) {
                return const Center(child: Text('Sem resultados agora.', style: TextStyle(color: text)));
              }
                ...
                itemBuilder: (context, i) {
                  ...
                  return Container(
                    ...
                    child: ListTile(
                      ...
                      leading: CircleAvatar(radius: 16, backgroundColor: Colors.black12, child: Icon(_selected.icon, size: 18, color: purple)),
                      title: Text(n.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: text, fontWeight: FontWeight.w600, fontSize: 12)),
                      subtitle: Padding(
                        padding: const EdgeInsets.only(top: 2),
                        child: Wrap(spacing: 8, crossAxisAlignment: WrapCrossAlignment.center, children: [
                          _Tag(label: _selected.label),
                          Text('${n.source}${n.createdAt != null ? ' • ${n.createdAt!.toLocal().toString().substring(0, 16)}' : ''}', style: TextStyle(color: text.withOpacity(0.7), fontSize: 10)),
                        ]),
                      ),
